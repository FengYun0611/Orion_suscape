...
The SUScape QA dataset has been integrated to provide more precise ground truth trajectories for L2 and collision rate evaluation. The QA dataset contains 12 different tasks (q1-q12), with **q7 (端到端轨迹预测)** being the most important for trajectory evaluation.

## QA Dataset Structure

```
data/sharegpt_dataset/
├── dataset_info.json
...
├── suscape_NQA_q5.json   # Scene description
├── suscape_NQA_q6.json   # Meta-action planning
├── suscape_NQA_q7.json   # End-to-end trajectory prediction ⭐ (KEY for L2 evaluation)
...
### 3. Check QA Data Loading

When the evaluation starts, you should see:
```
Loading QA dataset from data/sharegpt_dataset for tasks: ['q7']
Loaded 5000 QA items for task q7
```

During evaluation, when QA trajectory is used:
```
Using QA trajectory for scene scene-000000 frame 42
```
